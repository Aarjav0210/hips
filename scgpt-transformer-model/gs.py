...
# 5. Handle missing values (median imputation donor-wise for numeric cols)
...
def build_model_and_optim(config):
    """
    Build a TransformerNet + optimizer + scheduler given a config dict.
    """
...
def train_one_config(model, opt, sched, train_dl, val_dl, num_epochs=30, verbose=False):
    """
    Train a model for num_epochs, returning best validation loss.
    Uses donor-level aggregation inside batches.
    """
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs, y1, y2, donor_ids, cell_types) in enumerate(train_dl):
            ...
            # Map back to unique donors in this batch
...
for cfg in generate_configs(param_grid):
    print(f"\nTrying config: {cfg}")
    model_gs, opt_gs, sched_gs = build_model_and_optim(cfg)
    val_loss = train_one_config(
        model_gs, opt_gs, sched_gs,
        train_dl=train_dl,
        val_dl=val_dl,
...
print("Saved best hyperparameters to 'best_hparams.json'")

# ============================================================
# 3-FOLD DONOR-LEVEL CROSS-VALIDATION WITH BEST CONFIG
# ============================================================

print("\nStarting 3-fold cross-validation with best hyperparameters...")

donors_cv = np.array(train_donors + val_donors)

def make_dataloaders_for_donors(full_df, donors_train, donors_val, batch_size=128):
    train_df_fold = full_df[full_df['Donor ID'].isin(donors_train)].copy()
    val_df_fold   = full_df[full_df['Donor ID'].isin(donors_val)].copy()

    train_ds_fold = CellLevelDataset(train_df_fold, scgpt_cols, y1_cols, y2_cols)
    val_ds_fold   = CellLevelDataset(val_df_fold,   scgpt_cols, y1_cols, y2_cols)

    train_dl_fold = DataLoader(train_ds_fold, batch_size=batch_size,
                               shuffle=True, collate_fn=collate_with_donors)
    val_dl_fold   = DataLoader(val_ds_fold, batch_size=batch_size,
                               shuffle=False, collate_fn=collate_with_donors)
...
for fold_idx, (train_idx, val_idx) in enumerate(kf.split(donors_cv), start=1):
    fold_train_donors = donors_cv[train_idx].tolist()
    fold_val_donors   = donors_cv[val_idx].tolist()

    print(f"\n=== Fold {fold_idx} ===")
    print(f"Train donors: {len(fold_train_donors)}  Val donors: {len(fold_val_donors)}")

    train_dl_fold, val_dl_fold = make_dataloaders_for_donors(
        df, fold_train_donors, fold_val_donors, batch_size=128
    )

    model_cv, opt_cv, sched_cv = build_model_and_optim(best_config)

    fold_best_val = train_one_config(
        model_cv, opt_cv, sched_cv,
        train_dl=train_dl_fold,
        val_dl=val_dl_fold,
        num_epochs=NUM_EPOCHS,
        verbose=False
    )

    print(f"Fold {fold_idx} best validation loss: {fold_best_val:.4f}")
    fold_val_losses.append(fold_best_val)

print("\n3-fold CV validation losses:", fold_val_losses)
print(f"Mean CV val loss:  {np.mean(fold_val_losses):.4f}")
print(f"Std  CV val loss:  {np.std(fold_val_losses):.4f}")

# ============================================================
# FINAL TRAIN ON TRAIN+VAL WITH BEST CONFIG, THEN TEST
# ============================================================

print("\nTraining final model on (train + val) donors with best hyperparameters...")

final_train_donors = train_donors + val_donors

final_train_df = df[df['Donor ID'].isin(final_train_donors)].copy()
final_test_df  = df[df['Donor ID'].isin(test_donors)].copy()

final_train_ds = CellLevelDataset(final_train_df, scgpt_cols, y1_cols, y2_cols)
final_test_ds  = CellLevelDataset(final_test_df,  scgpt_cols, y1_cols, y2_cols)

final_train_dl = DataLoader(final_train_ds, batch_size=128,
                            shuffle=True, collate_fn=collate_with_donors)
# For training we can use the same loader as "val" just to drive scheduler; no test leakage
final_val_dl   = DataLoader(final_train_ds, batch_size=128,
                            shuffle=False, collate_fn=collate_with_donors)

final_model, final_opt, final_sched = build_model_and_optim(best_config)

_ = train_one_config(
    final_model, final_opt, final_sched,
    train_dl=final_train_dl,
    val_dl=final_val_dl,
    num_epochs=NUM_EPOCHS,
    verbose=True
)
...
torch.save({'model_state': final_model.state_dict(),
            'hparams': best_config},
           'best_transformer_model_final.pt')
print("Saved final model to 'best_transformer_model_final.pt'")
...
# Rebuild and load final model for clarity
model, _, _ = build_model_and_optim(best_config)
checkpoint = torch.load('best_transformer_model_final.pt', map_location=device)
...
with torch.no_grad():
    for donor_id in final_test_df['Donor ID'].unique():
        # Get all cells for this donor
        donor_mask = final_test_df['Donor ID'] == donor_id
        donor_data = final_test_df[donor_mask]
